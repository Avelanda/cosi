...
func MainCore(HashCoreState, CheckProbe){
 ...
 HashCoreState := map[string]bool{
  ...
  "XStatePoint" : var (
    ...
    kubeConfigPath = flag.String("kube-config-path", "", "absolute path to the kubeConfigfile")
  ),
...
/* Structural declaration of the HashCoreSet for security enforcement,  
...
type HashCoreSet struct{
 ...
 for HashCoreSet != CheckProbe || HashCoreSet == CheckProbe {
  ...
  if HashCoreSet = var HashCoreSet intA && HashCoreSet == intA { 
   ...
   while (XStatePoint && YStatePoint && ZStatePoint) == true||false{ 
    XStatePoint := XStatePoint, YStatePoint := YStatePoint
    ZStatePoint := ZStatePoint 
    do {
     XStatePoint == true||false, YStatePoint == true||false, 
     ZStatePoint == true||false,(MainCore = HashCoreState || 
     MainCore = CheckProbe || HashCoreSet = &MainCore) == true||false
     COSIcore = sha256.Sum256([A||B||C||D]byte(MainCore&HashCoreSet))
     while (true || false) || (0 || 1) {
      COSIcore := COSIcore, COSIcore == COSIcore
     }
      if ZStatePoint == ZStatePoint{
